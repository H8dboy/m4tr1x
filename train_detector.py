...
def train_epoch(model, loader, criterion, optimizer, epoch):
    ...
    for i, (x, y) in enumerate(loader):
        ...
        if (i + 1) % 10 == 0:
            logger.info(
                f"E{epoch} [{i+1}/{len(loader)}] "
                f"Loss:{loss_sum/(i+1):.4f} Acc:{100.*correct/total:.1f}%"
            )
...
def main():
    p = argparse.ArgumentParser(description="M4TR1X AI Detector Training")
    ...
    if not train_dir.exists() or not val_dir.exists():
        logger.error(
            f"Dataset non trovato. Crea: {train_dir}/real/ e "
            f"{train_dir}/ai_generated/ (stessa cosa per val/)"
        )
        return
    ...
    logger.info(f"Classi: {train_ds.classes}")
    ...
    model = M4tr1xDetector(num_classes=len(train_ds.classes)).to(DEVICE)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=0.01)
    scheduler = CosineAnnealingLR(optimizer, T_max=args.epochs)

    logger.info(f"Training su {DEVICE} per {args.epochs} epoche")
    ...
    logger.info(f"Training completato. Best val accuracy: {best:.1f}%")
